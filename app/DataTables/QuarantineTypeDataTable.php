...
class QuarantineTypeDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return datatables($query)
            ->addIndexColumn()
            ->addColumn('manage', 'quarantineTypes.btn.manage')
            ->rawColumns([
                'manage',
            ]);
    }
    ...
    {
        $btnAdd = [];
        $route = route('quarantineTypes.create');
        if (Auth::user()->can('manage quarantineTypes') != false) {
            $btnAdd = ['className' => 'btn btn-info ', 'text' => '<i class="fa fa-plus" ></i> ' . trans('dataTable.add.quarantineType'),
                'action' => " function(){
